...
export default function Clientes() {
  ...
  const { clientes, loading, fetchClientes, adicionarCliente, atualizarCliente, deletarCliente } = useClientesStore();
  ...
  const [formulario, setFormulario] = useState({ Empresa: '', Email: '', Telefone: '', Status: 'Ativo', Endereco: '' });

  useEffect(() => { fetchClientes(); }, []);

  const clientesFiltrados = clientes.filter((cliente) => {
    const matchBusca = cliente.Empresa?.toLowerCase().includes(busca.toLowerCase()) || cliente.Email?.toLowerCase().includes(busca.toLowerCase());
    const matchStatus = !statusFiltro || cliente.Status === statusFiltro;
    return matchBusca && matchStatus;
  });

  const handleAbrir = (cliente = null, e) => {
    if (e) e.stopPropagation();
    if (cliente) {
      setFormulario(cliente);
      setEditando(cliente.ID);
    } else {
      setFormulario({ Empresa: '', Email: '', Telefone: '', Status: 'Ativo', Endereco: '' });
      setEditando(null);
    }
    setModalOpen(true);
    setErro('');
  ...
  const handleSalvar = async (e) => {
    e.preventDefault();
    if (!formulario.Empresa) { setErro('Empresa é obrigatória'); return; }
  ...
  const getStatusColor = (status) => {
    const cores = { 'Ativo': 'bg-green-500/20 text-green-300', 'Inativo': 'bg-red-500/20 text-red-300', 'Pendente': 'bg-yellow-500/20 text-yellow-300' };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">{clientes.length} cliente{clientes.length !== 1 ? 's' : ''} cadastrado{clientes.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="relative">
          ...
          <input type="text" placeholder="Buscar por empresa ou email..." value={busca}
          ...
          <tbody>
            {loading ? (
              ...
              clientesFiltrados.map((cliente) => (
                <tr key={cliente.ID} onClick={() => nav(`/clientes/${cliente.ID}`)}
                  className="border-b border-slate-700 hover:bg-slate-700/50 transition-colors cursor-pointer">
                  <td className="px-6 py-4"><p className="text-white font-medium">{cliente.Empresa}</p></td>
                  <td className="px-6 py-4 text-slate-300">{cliente.Email || '-'}</td>
                  <td className="px-6 py-4 text-slate-300">{cliente.Telefone || '-'}</td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(cliente.Status)}`}>{cliente.Status}</span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={(e) => handleAbrir(cliente, e)}
                        className="p-2 hover:bg-slate-700 rounded-lg transition-colors text-cyan-400 hover:text-cyan-300">
                        <Edit2 size={18} />
                      </button>
                      <button onClick={(e) => handleDeletar(cliente.ID, e)}
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-8 max-w-md w-full">
            <h2 className="text-2xl font-bold text-white mb-6">{editando ? 'Editar Cliente' : 'Novo Cliente'}</h2>
            ...
            <form onSubmit={handleSalvar} className="space-y-4">
              <div>
                ...
                <input type="text" value={formulario.Empresa}
              ...
              <div>
                ...
                <input type="tel" value={formulario.Telefone || ''}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-200 mb-2">Endereço</label>
                <input type="text" value={formulario.Endereco || ''}
